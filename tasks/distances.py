...
def __get_distances(surnames_i, surnames_j):
    ...
    # save euclidean
    product["E_ij"] = math.sqrt(
        1 - distances_df.square_root_of_the_product_of_frequencies.sum()
    )

    product["I_ii"] = get_internal_isonymy(surnames_i)
    product["I_jj"] = get_internal_isonymy(surnames_j)

    product["N_ij"] = -1 * math.log(
        product["I_ij"] / math.sqrt(product["I_ii"] * product["I_jj"])
    )
...
def get_dapartamental_distances_in_meters(product, departments_shape_file):
    ...
    distances_gdf = (
        shape[["departamento_id", "geometry"]]
    ...
    for department_id in distances_gdf.departamento_id.unique():
        department_shape = distances_gdf[
            distances_gdf.departamento_id == department_id
        ]["geometry"].iat[0]
        distances_gdf[str(department_id)] = distances_gdf.centroid.distance(
            department_shape.centroid
        )
