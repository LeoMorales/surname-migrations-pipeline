...
def get_departmental_wright_m_2001(product):
    """"""
    ...
    output_df = df[work_columns].rename(
        columns={
    ...
    output_df["provincia_id"] = cleaning.rewrite_province_codes(
        output_df.loc[:, "CODPROV"]
    )
    output_df["department_id"] = cleaning.rewrite_department_codes(
        output_df["CODLOC"], output_df["provincia_id"]
    )

    output_df = output_df[["department_id", "population_2001", "ins", "fst", "m"]]
...
def get_departmental_wright_m_2015(upstream, product):
    ...
    population_df = population_df.rename(
        columns={"total_2015": "population_2015", "departamento_id": "department_id"}
    )

    wright_df = pandas.merge(population_df, isonymy_df, on="department_id")

    wright_df["m"] = isonymic.get_wright_m_vect(
        wright_df["population_2015"], wright_df["fst"]
    )
...
def get_departmental_wright_m_2021(upstream, product):
    ...
    """
    isonymy_df = pandas.read_parquet(upstream["get-departmental-isonymy-2021"])
    population_df = pandas.read_parquet(
        "/home/lmorales/resources/censo_proyeccion_poblacion.parquet"
    )
    ...
    population_df = population_df.rename(
        columns={"total_2021": "population_2021", "departamento_id": "department_id"}
    )

    wright_df = pandas.merge(population_df, isonymy_df, on="department_id")

    wright_df["m"] = isonymic.get_wright_m_vect(
        wright_df["population_2021"], wright_df["fst"]
    )
